...
| V5 | **Is the `0x03` commit frame really rare — and does press STYLE change it?** ⭐ | This is the central claim of [PR #4850](https://github.com/cagnulein/qdomyos-zwift/pull/4850) and it rests on **one session's data** (2 commits vs 32 terminators, ~6%). We have told a maintainer something about *his* users' hardware; it deserves a second measurement. **A live hypothesis we never tested: commit rate may depend on press duration** — session 13 was mostly short taps, so `0x03` may be reliable for longer presses and our "~6%" may really be "~6% for taps". | Controlled counts on `0c46be60`, same button throughout: **10 quick taps**, then **10 medium presses (~0.5 s)**, then **10 long holds (~2 s)**. Count `0x01`/`0x03`/`0x04`/`0x08` per group. Report commit rate **per group**, not pooled. Also note whether `0x04` always follows a commit and `0x08` never does — that pairing is the other half of the claim. |

> **Note on V5:** whatever the answer, it does **not** invalidate #4850's fix — triggering on the
> `0x01` held-stream is correct whether commits are 6% or 60%. What could change is the *explanation*
> we gave the maintainer, and that is worth getting right.
