...
> shifter map. **Open next: session 6** (🟡 planned) — **nRF sniff** of the app↔SB20 erg conversation
> (does the app bond? what does it write?) **+** the **power-topology Phase-2** multi-device capture, now that
> the sniffer is live. Then **G1/G2** (needs a USB flash) and the opportunistic **session 5** (track bike —
> paired XCadey+Assioma calibration). · bike-machine cold-start: [BIKE-SESSION-READY.md](../BIKE-SESSION-READY.md)

| # | Date | Status | Session (Plan + Actual) | Outcome |
|---|------|--------|--------------------------|---------|
| 6 | 2026-06-21 | 🟡 PLANNED | [sniff the app↔SB20 erg conversation + power-topology Phase 2](session-06-sniff-and-power-topology.md) | — *(headless nRF sniff: does the app bond / what does it write to erg + simultaneous multi-device capture to settle single-vs-dual-sided & true scale; runs on the bike laptop, Claude-driven)* |
