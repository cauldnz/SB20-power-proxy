...
|---|------|--------|--------------------------|---------|
| 11 | (planned) | 🟢 READY | [OBC on the bike: transmit → shifter-sink → web-config](session-11-obc-bike-test.md) | **The OBC feature stack on air** (PR #248 transmitter + Devmode + shifter-sink, PR #250 web config) — all host-tested + compiling but unproven on air. 3 standalone gates: **G1** a virtual `/obc/press` reaches a real BLE consumer (`obc_reader.py`, self-test PASS); **G2** ⭐ a real **SB20 paddle** press comes out as the mapped OBC action (the dual-role central+peripheral **coex + ~8 KB heap** is the risk); **G3** re-bind a button live via `/obc/buttons.json` and see the new action. Turnkey: `obc_reader.py` observer + curl endpoints, board online at `.165`. |
