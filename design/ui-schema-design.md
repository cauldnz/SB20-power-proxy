...
> **Design of record — not a status report.** Rescued to `main` 2026-07-24: this file lived only on the
> unmerged branch `docs/ui-architecture-review`, yet [`code/scripts/gen_bridge.py`](../code/scripts/gen_bridge.py)
> and [`.github/workflows/tests.yml`](../.github/workflows/tests.yml) both cite it by path. It is the
> rationale for the whole `ui-schema/` + `gen_bridge.py` system.
>
> **What shipped vs what this specifies** (see `decisions.md` 2026-07-13 and `architecture-remediation.md` R2):
> the schema (`ui-schema/bridge.json`), the generator, the committed goldens, `web/bridge-codec.js`, the
> Node parity test and the `bridge-parity` CI job all landed. **The one U2a step that did not** is the
> italicised clause below — *wire the SPA to import it* — which is why `web/index.html` still hand-codes
> its own `DataView` offsets. That gap is tracked as **R2a**; see the rollout section.
