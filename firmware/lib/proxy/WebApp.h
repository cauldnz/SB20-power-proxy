...
// The streaming dashboard served at GET /ui. A self-contained, dependency-free page that POLLS the
// status JSON at GET / once a second and renders power/cadence + a live chart entirely in the
// browser — so the ESP32 only serves the small JSON it already serves and all the UI cost lives on
// the phone. Static (no device-side templating), so it is just a constant; a host test guards that
// the essentials (the poll, the canvas, the fields) survive edits.
