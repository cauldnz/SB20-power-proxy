...
| 4 | **IDENT** | Fleet identity and bike affinity: a unique per-board *default* spoof identity (MAC-derived, never `62144`), a fleet table (board → identity → bike → pinned pedals → trainer full name), identity + source + trainer shown on `/status` and the ride screen, a `qa_board` check | desk, agent | #330 · forward-plan §8; session 13 G0; decisions 2026-09-23 | `Config.h` defaults every board to `Stages 62144` (bike 1's real crank); the Guition is `62145`, the C3 ride board's id |
| 5 | **S14-G0** | Session-14 desk pre-stage: both head units re-flashed from `main` with unique identities, the bike-2 inventory (SB20 #2 name/address, crank ids, battery), expected-topology cards, the capture rig proven live, restore values for both bikes | desk → hardware | #331 · `sessions/session-14-two-bike-first-rides.md` | the ride board was last flashed 07-26; nothing in the repo describes the second SB20 |
