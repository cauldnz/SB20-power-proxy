...
| 9 | **#323** | CI compiles `esp32-guition-live-ota` and `esp32s3-pio-live-ota` | desk, agent | #323 | CI compiles only the C3, CYD and nRF envs; every September change landed on an unguarded env |
...
5. **Two-rider motivation features** (owner wish): a shared dashboard or pace-match between the two
   head units; needs the fleet-status instrument below first.
6. **#291 decision → #247 G2** and the OBC seed-on-connect audit (decisions 07-26): buttons only;
   the qz/Peloton erg path needs none of it.
7. **#324** push the qz-fork branch and open the upstream PR (owner's public action).
8. **V1–V5 SB20 facts and the hold-to-repeat ride** (qz path; the table lives in session 14's annex).
9. **forward-plan §12** single-crank pairing test (session 14 stretch).
10. **R1e.2 CYD screen sweep** via the bench camera.
11. **#321** as a PC-side MCP adapter over the head unit's HTTP API (discover boards by mDNS).
12. **`fleet_status.py`**: poll N boards' `/status` and `/stats` (`perf_soak.py` is single-board).
13. **MeterCompare with two real Assioma sets** (session 14 stretch S2; today every number in that
    feature is fabricated).
14. **#334** IP + RSSI on the LCD Settings/More tab (re-filed from PR #255).
