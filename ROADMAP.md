...
|---|---|---|---|---|---|
| 1 | **PELOTON** | ⭐ **Owner wants this ASAP (2026-09-24).** Ride a Peloton **Power Zone** class and have our stack drive erg from the class's own targets, the way qz does: detect the live ride from Peloton's servers, read its power-zone target data, steer the trainer. **Most of this exists** — the on-device workout engine, `/workout`, the FTMS erg drive and `Segment.zone`/`pct_ftp` + `RiderProfile` are all built, so Peloton is a new workout **source**, not a new erg engine: a `from_peloton()` beside `from_zwo`/`from_fit`. **Phase 0 (capture a real PZ ride's API responses) before any code** — five questions fork the design, starting with "are targets live or only post-class?". Phase 1 desk-side adapter → `POST /workout`; Phase 2 on-device. Blocked on rider FTP (Next item 3) for two riders; needs a qz-interlock decision (never two erg controllers on one bike) | owner (Phase 0: a real ride + OAuth) → desk, agent | #342 · `code/findings/peloton-integration.md` (to be written) | promoted from Next 2026-09-24; the scoping it asked for is in #342 |
