...
| **C3 + 0.96" OLED** (AliExpress) | ESP32-C3 | base/STA `10:B4:1D:BA:C9:0C` | `303A:1001` (native USB-JTAG) | 128×64 @0x3C — **I2C SDA=5 / SCL=6 CONFIRMED**; controller SSD1306-vs-SH1106 TBD | New head-unit C3. 4 MB XMC flash. **Pins confirmed 2026-07-13** by the rewritten `c3-oled-probe` (scan at **50 kHz** — the earlier 100 kHz scan false-negatived — then it wrote the result to **NVS**, read back with `esptool read-flash 0x9000 0x5000` → `OLEDPROBE SDA=05 SCL=06 ADDR=3C`, fully autonomous, no eyes/serial). SSD1306 build (`esp32c3-oled96-live`) on the correct pins still drew **blank**, so the controller is likely **SH1106** — the probe now sweeps SSD1306↔SH1106 on the panel (one glance names it); build the winner with `esp32c3-oled96-live` (SSD1306) or `esp32c3-oled96sh-live` (SH1106). Its native-USB-JTAG **doesn't deliver `Serial` to the host** (esptool flashes fine; app output silent) — use the NVS-read-back / OLED channel, not serial. |
