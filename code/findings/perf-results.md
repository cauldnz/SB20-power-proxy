...
| 2026-06-17 | _pending flash_ | **OLED render → dedicated task** (off the hot loop) + render-on-change; OLED RSSI relabel `WiFi -XX` | — | — | — | — | — | — | — | ⏳ **PENDING MEASUREMENT** — code done + compiles, but the board sat at RSSI −80 and OTA wouldn't hold. Prediction: each 94 ms I²C send now blocks the OLED task (which yields), not the loop → **loop_max 94 ms → single-digit ms, stalls_50ms → ~0**. Flash near the AP / via USB to confirm + KEEP-or-revert |
