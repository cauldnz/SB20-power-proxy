...
- **[guition-board.md](guition-board.md)** — the **Guition JC3248W535** (ESP32-S3-N16R8, 3.5" **320×480 AXS15231B QSPI** + capacitive touch, 8 MB PSRAM, COM14): ported head-unit target (`esp32-guition*` envs) driven via the **vendored Espressif `esp_lcd_axs15231b`** driver (`firmware/lib/`) with a board-specific init table, TE sync and LVGL **full-refresh** mode + the port gotchas (GPIO8 = touch SCL not the LED!). The first QSPI-panel board; Phase-1 validated 2026-09-23. *(Arduino_GFX was the original plan and was abandoned — see the doc for the version pincer that killed it.)*
