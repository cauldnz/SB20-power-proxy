...
- **[on-device-workout-engine.md](on-device-workout-engine.md)** — *(✅ built: phases 1–4 on main, FTMS wire 2026-07-05; phase 5 = the real-SB20 ride, session 14)* the **on-device** workout executor: a structured workout uploaded over a `/workout` route, persisted, and run deterministically as the FTMS erg controller. Canonical format = compact JSON (1:1 with the Python `Segment`); FIT/ZWO are desk-side import formats. Backlog: [`forward-plan.md`](forward-plan.md) §14.
- **[erg-response-dyno-autotuner.md](erg-response-dyno-autotuner.md)** — *(RESEARCH)* the trainer-agnostic two-stage erg **dynamic-response** program: **Dyno** first measures an exact trainer/firmware plant (initial hardware: SB20 + Tacx NEO; future: KICKR), then an optional, evidence-gated, instant-bypass **Autotuner** may apply bounded plant compensation without changing the workout prescription. Distinct from static A/B meter bias. → governs future trainer adapters, dynamic identification, replay/simulation, and compensation seams.
- **[erg-response-dyno-autotuner-spec.md](erg-response-dyno-autotuner-spec.md)** — **OWNER-APPROVED PLANNING BASELINE; implementation paused.** Normative product/technical specification for the CYD + Assioma Dyno, versioned ERG protocols, trainer/firmware comparison, evidence-gated Autotuner, and separately gated hard-sprint envelope expansion.
- **[erg-response-dyno-autotuner-plan.md](erg-response-dyno-autotuner-plan.md)** — **OWNER-APPROVED PLANNING BASELINE; implementation paused.** Independently mergeable research/implementation slices from known-plant proof → FTMS simulator → SB20 → Tacx NEO → accepted baselines → optional compensation and sprint pressure-testing.
...
- **[meter-compare-visualization.md](meter-compare-visualization.md)** — *(✅ shipped 2026-07-16/20: the torque-axed Compare screen + the Python twin; §6 web plots and real two-meter data still open)* how to visualize live/offline A/B meter-error, emphasising the **torque domain** (power-only bins hide torque error). Bland–Altman + residual-vs-torque + 2-D power×cadence heatmap on the desk; a per-torque-band bar on the head-unit; the `MeterCompare.h` cadence-capture change that enables it. → governs `firmware/lib/proxy/MeterCompare.h` / `CompareService.h`, the LVGL Compare screen (`firmware/src/ui/LvglUi.cpp`), the web deep-dive (`web/index.html` via `GET /compare`), and the Python twin `src/sb20proxy/compare.py` + its CLI seam `scripts/compare_meters.py`.
...
- **[obc-protocol.md](obc-protocol.md)** — ⭐ **canonical for OpenBikeControl** (MIT): re-present the SB20 buttons to any OBC app (MyWhoosh, qz via #4504) over **BLE** (nRF + ESP) and **mDNS/TCP-UDP** (ESP). Pure codec `firmware/lib/proxy/Obc.h` — host-tested; the BLE + mDNS/TCP transports, the shifter-sink and the web config merged 2026-07-10 (#251) and were proven on air in session 13; the shifter-sink → qz path is blocked on #291. → issue #247.
- **[qz-upstream-contribution.md](qz-upstream-contribution.md)** — ⭐ **the state of the OBC-listener contribution to qdomyos-zwift** (the *consumer* half; the producer landed upstream in #4504): which fork branch is upstream-ready, the three review fixes applied vs our first cut, the **GPL/MIT boundary** (nothing flows back into this repo), the verbatim PR body + the claim boundaries we must not overstate, and the stale-bot deadline on issue #4791. **Read before touching `C:\repos\qdomyos-zwift`.**
- **[peloton-integration.md](peloton-integration.md)** — *(RESEARCH + PHASE 0 RECIPE, 2026-09-24; no code yet — ROADMAP Now #1, #342)* **Peloton Power Zone classes as a workout *source*** for the existing on-device engine. What qz does, clean-room, in prose (OAuth on Peloton's third-party host with a private client id; a 10 s/30 s *status* poll, then one download of the ride's `target_metrics_data.target_metrics` — inclusive `offsets`, `power_zone` `lower`/`upper` read as zone indices 1–7 → FTP multipliers, not chronological so it sorts; the class-clock alignment on `start_time` + the intro offset); what here it plugs into, verified file:line (`from_zwo`/`to_device_json` → `POST /workout/load`, `Segment.zone`/`pct_ftp` + `RiderProfile`, `WorkoutRuntime`); and the **turnkey Phase-0 capture run-sheet** — first-party session auth (no qz client id needed), the exact requests, `PELOTON-<ride-id>-<what>-<ts>.json` names, the redaction filter, what a good capture looks like, the acceptance checklist and the 11 questions it must answer. → will govern `workout/importers.py::from_peloton()` (Phase 1) and the on-device fetch (Phase 2). **Read before any Peloton work.**
...
- **[advanced-board-s3-touch.md](advanced-board-s3-touch.md)** — the Waveshare ESP32-S3-Touch-LCD-1.47 board (boots with the full LVGL touch UI, verified 2026-07-05) plus the original **"advanced" hardware tier** ideas: dual-core kills the C3 coex hang, a touch head-unit (no phone), SD capture — reusing the existing proxy core.
...
- **[guition-board.md](guition-board.md)** — the **Guition JC3248W535** (ESP32-S3-N16R8, 3.5" **320×480 AXS15231B QSPI** + capacitive touch, 8 MB PSRAM, COM14): ported head-unit target (`esp32-guition*` envs) driven via the **vendored Espressif `esp_lcd_axs15231b`** driver (`firmware/lib/`) with a board-specific init table, TE sync and LVGL **full-refresh** mode + the port gotchas (GPIO8 = touch SCL not the LED!). The first QSPI-panel board; Phase-1 validated 2026-09-23. *(Arduino_GFX was the original plan and was abandoned — see the doc for the version pincer that killed it.)* ⚠️ **The model name is reported (owner's AliExpress order), not read off the PCB, and the variant suffix is unknown** — the hardware facts and pin map are measured; see the doc's "Provenance" box.
...
- **[track-launch-control-research.md](track-launch-control-research.md)** — *(research / interview complete)* **Track cycling launch control**: coach timing board, bike-mounted nRF IMU/power capture, local acoustic start timing, one-run data lifecycle, validation gates, and ordered product decisions.
- **[track-launch-ble-transfer.md](track-launch-ble-transfer.md)** — *(research)* **BLE launch-run download throughput and reliability**: current framing and exact payload sizes, MTU/DLE/PHY/coexistence constraints, resumable durable-transfer design, and the required hardware benchmark.
- **[track-launch-control-spec.md](track-launch-control-spec.md)** — *(APPROVED BASELINE; implementation PAUSED — ROADMAP.md Parked)* **V1 build contract** for launch-control hardware, workflow, timing, capture, deep modules, state models, storage, UI, security, analysis confidence and acceptance.
- **[track-launch-control-plan.md](track-launch-control-plan.md)** — *(APPROVED BASELINE; implementation PAUSED — ROADMAP.md Parked)* **Evidence-gated delivery sequence** from generated contract and pure modules through Waveshare/nRF bring-up, durable BLE transfer, coach UI, static-bike proof and velodrome validation.

## Also in here
- **[captures/README.md](captures/README.md)** — the CI-guarded index of every committed capture file (the canonical lossless record — never edit one; `code/tests/test_captures_index.py` fails on an unindexed file).
- **perf/** — the soak-run JSONL behind [perf-results.md](perf-results.md). **annotations/** — committed segment labels over captures (`sb20proxy.analysis.annotations`).
