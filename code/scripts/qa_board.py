...
with the same sb20proxy.ble.cps codec the firmware mirrors.
    ...
    # a board on its MAC-derived default identity ("Stages 9NNNN", #330) needs no --spoof-name:
    python code/scripts/qa_board.py --no-flash --connect
    # ...or name it explicitly / from its base MAC (esptool read-mac, BOARDS.md) / from its /status:
    python code/scripts/qa_board.py --no-flash --spoof-name "Stages 62145"
    python code/scripts/qa_board.py --no-flash --mac A4:CB:8F:DA:E9:CC
    python code/scripts/qa_board.py --no-flash --ip 192.168.1.50          # also pulls /status
    # a board DELIBERATELY standing in for bike 1's real crank (the single-right-crank rescue):
    python code/scripts/qa_board.py --no-flash --spoof-name "Stages 62144" --crank-rescue

    # full pre-ship gate: flash, then accept:
    python code/scripts/qa_board.py --port COM10 --env esp32c3-oled-live --connect

The card FAILS a board advertising one of bike 1's real crank ids (Stages 62144 / 4963) unless
--crank-rescue says that is intended, and a name that more than one advertiser is using.
...
from sb20proxy.qa.acceptance import (  # noqa: E402
...
async def _scan(spoof_name: str, timeout: float) -> tuple[bool, list[str], list[str]]:
    """Scan for the spoof advert. Returns (seen?, names_seen, addresses_of_EVERY_match).

    Every matching address is kept, not just one: two boards (or a board and a real crank) on one
    identity is exactly the collision the acceptance card must catch (#330)."""
    from bleak import BleakScanner
    seen_names: list[str] = []
    match_addrs: list[str] = []
    ...
    return bool(match_addrs), seen_names, match_addrs
...
def main() -> int:
    ...
    p.add_argument("--spoof-name", default=None,
                   help="the crank name the board should advertise. Default: --mac's derived name, "
                        "else /status's identity (with --ip), else any MAC-derived default "
                        f"('{DERIVED_IDENTITY_PREFIX}' prefix)")
    p.add_argument("--mac",
                   help="the board's base MAC (esptool read-mac / BOARDS.md): expect its "
                        "MAC-derived default identity")
    p.add_argument("--crank-rescue", action="store_true",
                   help="this board deliberately stands in for one of bike 1's real cranks "
                        "(Stages 62144 / 4963): accept that identity instead of failing on it")
    ...
    p.add_argument("--no-flash", action="store_true",
                   help="don't flash; just accept a running board")
    ...
    spoof_name: str | None = a.spoof_name
    if spoof_name is None and a.mac:
        spoof_name = default_spoof_name(a.mac)
        print(f"expecting the MAC-derived default identity for {a.mac}: '{spoof_name}'")

    flash_ok: bool | None = None
    if not a.no_flash:
        if not a.port:
            p.error("give --port to flash, or pass --no-flash")
        flash_ok = _flash(a.env, a.port, spoof_name or DERIVED_IDENTITY_PREFIX)
        if not flash_ok:
            print("flash failed — board is not shippable")

    # /status first (after any flash): with no name given it tells us which identity to expect.
    status = _fetch_status(a.ip, timeout=5.0) if a.ip else None
    if spoof_name is None and status and status.get("identity"):
        spoof_name = str(status["identity"])
        print(f"expecting the identity /status reports: '{spoof_name}'")
    if spoof_name is None:
        spoof_name = DERIVED_IDENTITY_PREFIX
        print(f"no --spoof-name/--mac/--ip: expecting any MAC-derived default ('{spoof_name}...')")

    advert_seen = advert_names = match_addrs = None
    if flash_ok is not False:  # don't bother scanning if the flash itself failed
        print(f"scanning {a.scan_timeout:.0f}s for '{spoof_name}' ...")
        advert_seen, advert_names, match_addrs = asyncio.run(
            _scan(spoof_name, a.scan_timeout))

    cps_powers = None
    if a.connect and match_addrs:
        print(f"connecting {match_addrs[0]} to sample {a.frames} CPS frame(s) ...")
        try:
            cps_powers = asyncio.run(_sample_cps(match_addrs[0], a.frames, timeout=15.0))
    ...
    report = evaluate(
        expected_spoof_name=spoof_name, advert_seen=advert_seen,
        advert_names=advert_names, advert_matches=match_addrs, status=status,
        cps_powers=cps_powers, flash_ok=flash_ok, crank_rescue=a.crank_rescue,
    )
    ...
    if a.json:
        ...
        print()
        print(report.render(title=f"Board acceptance ({spoof_name})"))
