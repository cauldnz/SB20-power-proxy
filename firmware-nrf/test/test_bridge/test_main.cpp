...
#include "bridge_golden_gen.h"  // GENERATED by code/scripts/gen_bridge.py from ui-schema/bridge.json
...
// ================= Schema golden vectors (locks Proto.h to ui-schema/bridge.json) =============
// Each test builds a struct with the SAME deterministic sample values gen_bridge.py uses, packs it
// via Proto.h, and asserts the bytes equal the generated golden hex. A Proto.h offset/scale change
// without a matching schema edit fails here; the JS codec (Node parity test) asserts the same golden.
static void assertGolden(const char* label, const char* hex, const uint8_t* b, size_t n) {
    TEST_ASSERT_EQUAL_UINT_MESSAGE(n * 2, strlen(hex), label);
    auto nib = [](char c) -> int { return c <= '9' ? c - '0' : (c | 0x20) - 'a' + 10; };
    for (size_t i = 0; i < n; ++i) {
        const uint8_t want = (uint8_t)((nib(hex[i * 2]) << 4) | nib(hex[i * 2 + 1]));
        TEST_ASSERT_EQUAL_HEX8_MESSAGE(want, b[i], label);
    }
}

void test_golden_status(void) {
    StatusPacket s;
    s.srcConnected = true; s.outAdvertising = false; s.recording = true;
    s.srcIsAnt = false; s.outIsAnt = true;
    s.srcPowerW = -321; s.outPowerW = -321; s.cadenceRpm = -321;
    s.balancePct = -7; s.batteryPct = 42; s.scaleMilli = 1234; s.offsetDeciW = 150;
    s.recSamples = 123456; s.uptimeS = 4321;
    uint8_t b[STATUS_LEN]; packStatus(s, b);
    assertGolden("Status", bridge_golden::Status, b, STATUS_LEN);
}
void test_golden_config(void) {
    ConfigPacket c;
    c.srcIsAnt = true; c.outIsAnt = false; c.singleSided = true; c.spoof = false;
    c.scaleMilli = 1234; c.offsetDeciW = 150;
    strcpy(c.srcFilter, "SRCF"); strcpy(c.outName, "OUTN");
    uint8_t b[CONFIG_LEN]; packConfig(c, b);
    assertGolden("Config", bridge_golden::Config, b, CONFIG_LEN);
}
void test_golden_buttons(void) {
    ButtonsPacket bt; bt.enabled = true; for (int i = 0; i < 6; ++i) bt.act[i] = 42;
    uint8_t b[BUTTONS_LEN]; packButtons(bt, b);
    assertGolden("Buttons", bridge_golden::Buttons, b, BUTTONS_LEN);
}
void test_golden_recstate(void) {
    uint8_t b[RECSTATE_LEN]; packRecState(RecState::Recording, 42, 123456, 123456, b);
    assertGolden("RecState", bridge_golden::RecState, b, RECSTATE_LEN);
}
void test_golden_calstate(void) {
    const int cov[6] = {42, 42, 42, 42, 42, 42};
    uint8_t b[CALSTATE_LEN]; packCalState(CalWireState::Collecting, 4321, 4321, 150, cov, true, b);
    assertGolden("CalState", bridge_golden::CalState, b, CALSTATE_LEN);
}
void test_golden_wkstate(void) {
    // flags: loaded b0 | paused b2 | ergControlled b4 = 0x15
    uint8_t b[WKSTATE_LEN]; packWkState(0x15, -321, 42, 42, 4321, -321, 4321, -321, b);
    assertGolden("WkState", bridge_golden::WkState, b, WKSTATE_LEN);
}
void test_golden_curve(void) {
    CurvePoint pts[2] = {{4321, 1234}, {4321, 1234}};
    uint8_t b[2 + 2 * 4]; size_t n = packCurve(pts, 2, b);
    assertGolden("Curve", bridge_golden::Curve, b, n);
}
void test_golden_scanlist(void) {
    ScanEntry e[2];
    strcpy(e[0].name, "NAME"); e[0].rssi = -7; e[0].flags = 0x05;
    strcpy(e[1].name, "NAME"); e[1].rssi = -7; e[1].flags = 0x05;
    uint8_t b[2 + 2 * SCAN_SLOT]; size_t n = packScanList(e, 2, b);
    assertGolden("ScanList", bridge_golden::ScanList, b, n);
}

// ================= ANT+ Bike Power master scheduler (AntMasterScheduler.h) =====================
// Mirrors the page schedule of the Python StagesAntTarget._next_page (targets/stages_ant.py).

void test_ant_master_leads_with_commons() {
    nrfant::AntMasterScheduler sch;  // commonsEvery = 121
    sch.begin();
    uint8_t p[8];
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x50, p[0]);   // manufacturer
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x51, p[0]);   // product
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x52, p[0]);   // battery
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x10, p[0]);   // then the power data page
}

void test_ant_master_power_pages_accumulate() {
    nrfant::AntMasterScheduler sch;
    sch.begin();
    uint8_t p[8];
    for (int i = 0; i < 3; ++i) sch.nextPage(200, 90, -1, p);  // drain the lead commons
    sch.nextPage(200, 90, -1, p);
    TEST_ASSERT_EQUAL_HEX8(0x10, p[0]);
    TEST_ASSERT_EQUAL_UINT8(1, p[1]);                                 // eventCount
    TEST_ASSERT_EQUAL_UINT8(90, p[3]);                                // cadence
    TEST_ASSERT_EQUAL_UINT16(200, (uint16_t)(p[6] | (p[7] << 8)));    // instantaneous power
    TEST_ASSERT_EQUAL_UINT16(200, (uint16_t)(p[4] | (p[5] << 8)));    // accumulated power
    sch.nextPage(200, 90, -1, p);
    TEST_ASSERT_EQUAL_UINT8(2, p[1]);                                 // eventCount++
    TEST_ASSERT_EQUAL_UINT16(400, (uint16_t)(p[4] | (p[5] << 8)));    // accum += 200
}

void test_ant_master_commons_interspersed() {
    nrfant::AntMasterScheduler sch(3);  // commons every 3
    sch.begin();
    uint8_t p[8];
    for (int i = 0; i < 3; ++i) sch.nextPage(150, 80, -1, p);  // lead commons
    sch.nextPage(150, 80, -1, p); TEST_ASSERT_EQUAL_HEX8(0x10, p[0]);  // data
    sch.nextPage(150, 80, -1, p); TEST_ASSERT_EQUAL_HEX8(0x10, p[0]);  // data
    sch.nextPage(150, 80, -1, p); TEST_ASSERT_EQUAL_HEX8(0x50, p[0]);  // commons burst resumes
}

void test_ant_master_calibration_response_wins() {
    nrfant::AntMasterScheduler sch;
    sch.begin();
    sch.setCalibrationOffset(903);
    sch.queueCalibration(2);
    uint8_t p[8];
    sch.nextPage(200, 90, -1, p);
    TEST_ASSERT_EQUAL_HEX8(0x01, p[0]);                               // cal page wins over pending commons
    TEST_ASSERT_EQUAL_HEX8(0xAC, p[1]);                               // manual-zero success
    TEST_ASSERT_EQUAL_UINT16(903, (uint16_t)(p[6] | (p[7] << 8)));    // the captured offset (0x0387)
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x01, p[0]); // 2nd cal broadcast
    sch.nextPage(200, 90, -1, p); TEST_ASSERT_EQUAL_HEX8(0x50, p[0]); // then the pending commons resume
}

int main(int, char**) {
    UNITY_BEGIN();
    RUN_TEST(test_golden_status);
    RUN_TEST(test_golden_config);
    RUN_TEST(test_golden_buttons);
    RUN_TEST(test_golden_recstate);
    RUN_TEST(test_golden_calstate);
    RUN_TEST(test_golden_wkstate);
    RUN_TEST(test_golden_curve);
    RUN_TEST(test_golden_scanlist);
    ...
    RUN_TEST(test_ant_master_leads_with_commons);
    RUN_TEST(test_ant_master_power_pages_accumulate);
    RUN_TEST(test_ant_master_commons_interspersed);
    RUN_TEST(test_ant_master_calibration_response_wins);
