...
- **Find and read the doc before you build — don't rebuild what's documented.** Before building tooling for, or judging the readiness/availability of, any subsystem (a radio, a protocol, a capture path), find its canonical doc via the index **[`code/findings/README.md`](code/findings/README.md)** and read it — and use the existing scripts it names rather than re-deriving them. When you add/rename/retire a findings doc, update that index in the same change; CI (`code/tests/test_findings_index.py`) fails if a doc is missing from the index or the index links somewhere dead, so it stays current automatically. *(Session 9: a capture-rig gate **and** an "install Npcap" remediation were both built on assumptions because `nrf-sniffer.md` — which documented the real path + the existing `sniff_ble.py` — was never opened. Hours lost to a doc that was right there.)*
