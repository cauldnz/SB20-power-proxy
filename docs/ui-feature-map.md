...
**Status: LIVING (first cut 2026-09-24; issue #345).** Every user-facing feature on every surface: what
...
the owner's call (blank until confirmed). Defects found while mapping are marked ⚠ and listed in §3.

### 2a. Onboarding and WiFi

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F01 | Join the board to WiFi (captive portal on `Setup-XXXX`, `172.29.4.1`: network list, password, save → reboot) | LCD: QR join screen + SSID/PIN; OLED: SSID/PIN rows | — | portal page | HW: C3 07-11, CYD 07-25, Guition 09-23; a phone scanning the QR: never recorded | D (QR/PIN) + portal page | |
| F02 | Forget WiFi / re-provision | — (no BOOT-button path exists) | — | `POST /forget` (legacy only) | never | D (More → Forget WiFi, confirm) + W | |
| F03 | Ride-mode WiFi-off until power-cycle | — | — | `GET/POST /wifi/off` (legacy) | desk (GET only) | D (More toggle) + W | |

### 2b. Source and identity

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F04 | Pick the source meter from a scan list, pinned by address | Setup screen: filtered list (CPS/FTMS only), tap + Save → reboot | list shown; ⚠ cannot pin by address (`mergeSpaConfigForm` never sets `meterAddress`) | `/setup` list + hidden `addr`, Save → reboot | HW: every session (legacy, LCD) | D+W | |
| F05 | Rescan for devices | Setup → Rescan (scan-window boost) | ⚠ `POST /setup/scan` is a phantom (route is GET-only; the call is dead code) | `GET /setup/scan` | HW: CYD 07-03 | D+W | |
| F06 | Source **name filter** (when not pinned) | — | `src_filter` (inert once an address is pinned) | `name` field | HW 07-11 (HTTP), 07-11 (BLE) | W | |
| F07 | Single-sided ×2 (a surviving R crank) | — | checkbox | checkbox | HW BLE 07-11; HTTP sent 07-11 | W | |
| F08 | Mode: spoof ↔ corrector (reboots) | More: read-only row | selector + reboot hint | none to set; `/more` read-only | HW 07-11 | W (device shows it) | |
| F09 | Spoof identity: name + serial | More: read-only row | name (locked to `Stages 62144` in spoof mode ⚠ hard-coded lock string); no serial | name + serial | HW BLE 07-11; HTTP preserved 07-11 | W (device shows it); default becomes per-board (#330) | |
| F10 | Trainer (erg target) selection | Setup: FTMS rows; More → Trainer ⚠ always `not set` (#346) | ⚠ "Set trainer" → `POST /workout/trainer` 404 (#347) | `/setup` FTMS tap-list | HW legacy C3 07-05; LCD pick twin-proven 07-05 | D+W | |
| F11 | Reset source + identity to defaults | — | — | `POST /setup/reset` + confirm | CSRF probe only | W | |
| F12 | Spoof radio BLE / ANT+ (nRF) | — | BLE: selector, ANT disabled | — | never (S340-gated) | W (BLE) | |

### 2c. Ride display

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F13 | Live power hero, cadence, L/R balance | Ride: hero + cadence/balance cards; OLED rows | hero (3.4 rem) + Live card, 1 Hz; BLE 2 Hz | `/`: hero 4.6 rem + cards, 1 Hz | HW: LCD every board; SPA/HTTP mock values 07-05; legacy every session | D+W | |
| F14 | Power history chart | Ride: 48-point line chart | ⚠ none | `/`: 90-sample canvas | HW LCD | D+W (add to `/app` for ride use) | |
| F15 | Source / output names, link state, uptime, RSSI | Ride title bar + details pop-down (IN RSSI, OUT uptime) | source-link + uptime rows | `/` title + detail cards | HW: CYD 07-03, S3 07-04, Guition 09-23 | D+W | |
| F16 | Current erg target, segment, time left **on the ride screen** | ⚠ only in the canvas twin (`RideView` strip never drawn by LVGL); LVGL shows it on the Workout screen only | ⚠ in the Workout card (7th of 10 cards), not the hero | none | desk | D+W (both need it on the ride view) | |
| F17 | Erg link status (connected / controlled) | Workout screen erg line (4 states) | ⚠ "searching…" forever on HTTP (missing `erg_*` fields, #347); real on BLE | none | LCD: twin 07-05 | D+W | |
| F18 | Keep the display awake through a ride; survive a board reboot | n/a (device) | ⚠ no Wake Lock; HTTP has no reconnect (errors swallowed, green dot never revoked); BLE reconnects with backoff | n/a | S13 R13 (BLE) | W (required for principle 3) | |
| F19 | Identity + mode shown as text | More rows | `devName` = hostname only; mode in Settings | `/` title, `/more` rows | HW 07-02 (legacy) | D+W | |

### 2d. Erg and workouts

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F20 | Load a preset (4×8, SS 3×12, VO2 5×3, Endurance 45) | Workout picker (4 buttons) | Workout card preset | `/workout` picker | HW: CYD 07-03, HTTP 07-02, twin 07-05 | D+W | |
| F21 | Start / Pause / Resume / Stop | Workout console buttons | verbs | `/workout` | HW: baseline 07-27; twin 07-05; **never on a real SB20** | D+W | |
| F22 | Skip segment | Skip button | ⚠ none (BLE has no skip either) | `POST /workout/skip` | LCD twin | D+W | |
| F23 | Unload / change workout | Change button (when loaded, not running) | ⚠ none | none | LCD 07-03 | D+W | |
| F24 | Import a custom workout (ZWO / FIT / JSON) | — | ⚠ none | `/workout/load` textarea; `import_workout.py --post` | legacy + tooling | W | |
| F25 | Per-rider FTP (scales `%FTP` and zone segments; presets assume 250 W) | — | — | — | never (ROADMAP Next 3) | W (set) + D (shown) | |
| F26 | Peloton class as the workout source (#342) | — | — | — | not built | W (login, class pick) → D (runs it) | |
| F27 | Shifter bias ±10 W | via the SB20 buttons (OBC actions 8/9) | ⚠ `POST /workout/bias` 404 (#347); works on BLE | none | BLE bench 07-05 | D (buttons) + W | |
| F28 | Workout progress: profile bar chart, next block, total clock | Workout console (bar chart, `N of M`, clock) | segment + time left only | `/workout`: profile, next, total | HW LCD 07-03/04 | D+W | |

### 2e. Calibration and corrector mode

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F29 | Meter-to-meter calibration wizard (pick DUT + ref, collect, fit, save → corrector) | ⚠ Calibrate screen's only button (`Connect + start`) does nothing (#346) | ⚠ card dead on HTTP (`/calibrate/state` phantom; Start posts a name where addresses are required, #347); works on BLE | `/calibrate` 3-state wizard (addresses, coverage bars, fit table) | legacy: desk only; the corrector ride never run | W (device shows state only) | |
| F30 | Portable calibration profile export / import (`/curve`) | — | profile card | none | HW 07-05 (HTTP), 07-11 (BLE) | W | |
| F31 | Scalar scale / offset (nRF only) | — | BLE inputs; hidden on the ESP32 | — | HW 07-11 (hidden correctly) | W (BLE) | |
| F32 | Fit review (points, residual) | — | "N points" | full table | desk | W | |

### 2f. Compare (A/B meters)

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F33 | A/B compare: verdict, bias-by-torque chart, Bland–Altman | More → Compare screen (12 bands); ⚠ meter B is simulated (A×1.11) | Compare card, 3 s poll; ⚠ hook only on LCD builds, so the C3 says "waiting…" forever | none | desk (host tests; no on-panel record) | W (deep dive) + D (verdict) — revisit after real two-meter data | |

### 2g. OBC buttons

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F34 | SB20 button → action binding; enable the shifter sink | — | buttons card (`/obc/buttons.json`, live) | `GET /obc` help text | HW: JSON via curl (sessions 11/13); the card itself never | W | |
| F35 | Devmode / shifter-sink toggles; virtual press | — | — | `POST /obc/{devmode,shifter}/…`, `GET /obc/press` (curl) | HW curl sessions 11/13 | T (curl) or W Settings | |

### 2h. Diagnostics, report, updates, admin

| id | Feature | Device today | Web today | Legacy today | Verified | Proposed | Decision |
|---|---|---|---|---|---|---|---|
| F36 | Tester report: review on device → download / copy / email (`/report`, `/diag`) | — (report-v2 mockup never built) | — | `/report`, `/diag` | desk; beta docs point here | W | |
| F37 | `/log`, `/stats`, `/status` (instruments) | — | client-side log card only | footer links; curl | HW every session | T | |
| F38 | Firmware version + build SHA visible | ⚠ Version row exists but is unreachable (#346); OLED none | ⚠ none | `/` footer (`fw`); `/status` | HW `/status` | D+W (session 14 G0 needs it per board) | |
| F39 | Push OTA / signed pull / nRF DFU | — | — | none (push OTA is `flash.ps1`) | HW OTA 09-23 | T | |
| F40 | Reboot the board | — (every Save reboots) | — (every Apply reboots) | none explicit | — | D (More → Reboot) + W | |
| F41 | Backlight brightness | More → Bright ⚠ stuck at 25 % (#346) | n/a | n/a | never (defect) | D | |
| F42 | Touch calibration (CYD only) | More → Touch cal; BOOT ≥ 1 s; first boot | n/a | n/a | HW 07-03; refactor 07-26 not re-run | D | |
| F43 | Watty Birds | serial `WATTY` only (no menu entry) | n/a | n/a | boot-safety 07-25; gameplay never recorded | D (hidden entry) | |
| F44 | IMU track recording (nRF) | LED colour | BLE recording card | — | live sampling 07-04 | W (BLE) | |
...
| F04 | bench | `fake_meter.py` + a second CPS advertiser; camera + `TAP`; `/app` | the picker lists only CPS/FTMS devices; a tap + Save pins the *address* (`/status` shows it) on the device and on `/app` |
...
| F26 | bike | Peloton Phase 0 → 1 (#342) | a real Power Zone class's timeline runs on the head unit |
| F27 | bench | SB20 buttons or `obc_reader.py`; `/app` | ±10 W reaches the sim from the buttons and from `/app` (needs #347) |
...
| F34–F35 | bench | `/app` buttons card; `obc_reader.py` | a re-bound button emits the new action live |
...
- **Placement changes** (a feature moved on or off a surface) become ROADMAP Next items; the legacy
  pages retire in one PR once every web-placed row is green on `/app`.
